...
public class Main {
    ...
    public static void game () {
        ...
        int plScore = 0, dlScore = 0, round = 1;
        int win;
        Scanner sc = new Scanner(System.in);
        win = round(round, sc);
        if (win == 1) {
            plScore++;
            System.out.printf("%nВы выиграли раунд! Счет %d:%d в вашу пользу.%n", plScore, dlScore);
        }
        else if (win == 0) {
            dlScore++;
            System.out.printf("%nВы проиграли раунд! Счет %d:%d в пользу дилера.%n", plScore, dlScore);
        }
        else {
            plScore++;
            dlScore++;
        ...
        sc.close();
        System.out.printf("\nИгра окончена!\nСчет %d:%d%s%n", plScore, dlScore, plScore > dlScore ? " в вашу пользу." : (plScore == dlScore) ? ". Ничья!" : " в пользу дилера.");
